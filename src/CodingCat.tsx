...
export const CodingCat = () => {
  ...
  const pawLeftUpRef = React.useRef<SVGGElement | null>(null)
  const pawLeftDownRef = React.useRef<SVGPathElement | null>(null)
  const pawRightUpRef = React.useRef<SVGGElement | null>(null)
  const pawRightDownRef = React.useRef<SVGPathElement | null>(null)
  const terminalCodeRef = React.useRef<SVGGElement | null>(null)

  const animationRef = React.useRef<CodingCatAnimation | null>(null)
  ...
  React.useLayoutEffect(() => {
    ...
    if (
      ...
      musicNotesRightContainerRef.current &&
      musicNotesLeftContainerRef.current &&
      pawLeftUpRef.current &&
      pawRightDownRef.current &&
      pawLeftDownRef.current &&
      pawRightUpRef.current &&
      terminalCodeRef.current
    ) {
      animationRef.current = new CodingCatAnimation({
        animationContainerEl: containerRef.current,
        musicNotes: {
          rightContainer: musicNotesLeftContainerRef.current,
          leftContainer: musicNotesRightContainerRef.current
        },
        paws: {
          right: {
            up: pawRightUpRef.current,
            down: pawRightDownRef.current
          },
          left: {
            up: pawLeftUpRef.current,
            down: pawLeftDownRef.current
          }
        },
    ...
    <CodingCatContainer className="coding-cat-animation" ref={containerRef} width="80%" height="80%">
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 783.55 354.91">
        <g id="bongo-cat">
          ...
          <g className="music music-right" ref={musicNotesRightContainerRef}>
            <g className="note">
              <g>
          ...
          <g className="paw paw-right">
            <path ref={pawRightDownRef} className="down" d="M289.1,181.7c-12.1,9.8-20.6,20.7-20.7,32.1-.2,9,3.8,20.4,13.3,25.2s20.1.6,29.6-3.4c13.4-5.7,23.9-14.6,29.4-21.5"></path>
            <g ref={pawRightUpRef} className="up">
          ...
          <g ref={terminalCodeRef} className="terminal-code">
          ...
          <g className="paw paw-left">
            <g className="up" ref={pawLeftUpRef}>
            ...
            <path ref={pawLeftDownRef} className="down" d="M534.1,231.4c-19.7,6-32.9,18.4-34.2,29.1a30.1,30.1,0,0,0,1.7,14.1,24.8,24.8,0,0,0,6.1,8.8c6,5.1,16.8,4,38-3.9a288.7,288.7,0,0,0,46.5-22.1"></path>
          </g>
          ...
          <g className="music music-left" ref={musicNotesLeftContainerRef}>
            <g className="note">
              <g>
